...
class RelatedQuestions extends Extended
{
    /**
     * Core registry
     *
    ...
    protected function _construct()
    {
        ...
        if ($this->getQuestion() && $this->getQuestion()->getQuestionId()) {
            $this->setDefaultFilter(['in_questions' => 1]);
        }
        if ($this->isReadonly()) {
    ...
    {
        $collection = $this->_question->getCollection();
        if ($this->isReadonly()) {
            $questionIds = $this->_getSelectedProducts();
    ...
    {
        return $this->getQuestion() && $this->getQuestion()->getCrosssellReadonly();
    }
    ...
    public function getGridUrl()
    {
        return $this->_getData(
            ...
            'loffaq/question/relatedquestionsGrid',
            ['_current' => true]
        );
    }
    ...
    protected function _getSelectedProducts()
    {
        ...
        if (!is_array($products)) {
            $products = array_keys($this->getSelectedQuestionProducts());
        }
        return $products;
    }
     ...
     * @return array
     */
    public function getSelectedQuestionProducts()
    {

        $products = [];
        if(!empty($this->_coreRegistry->registry('current_question')->getData('relatedquestions'))){
            foreach ($this->_coreRegistry->registry('current_question')->getData('relatedquestions') as $product) {
                $products[$product['relatedquestion_id']] = ['question_position' => $product['relatedquestion_id']];
            }
        }
        return $products;
    }
